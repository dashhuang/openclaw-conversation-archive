...
DEFAULT_ARCHIVE_DIR = Path("logs/message-archive-raw")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Search workspace-local conversation archive JSONL."
    )
    ...
    parser.add_argument("--archive-root", default=str(DEFAULT_ARCHIVE_DIR), help="Archive root to search.")
    parser.add_argument("--limit", type=int, default=8, help="Max hits to print (default: 8).")
    parser.add_argument("--json", action="store_true", help="Emit JSON instead of text.")
    return parser.parse_args()


def iter_entries(archive_root: Path):
...
def main() -> int:
    ...
    if not archive_root.exists():
        print(f"archive directory missing: {archive_root}", file=sys.stderr)
        return 2

    results = [entry for entry in iter_entries(archive_root) if matches(entry, args)]
